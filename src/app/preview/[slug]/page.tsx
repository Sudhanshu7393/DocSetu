...
export default function PreviewPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.text("DocSetu", 14, 12);
      pdf.setFontSize(8);
      pdf.setFont("helvetica", "normal");
      pdf.text("Making Official Documents Easy  |  docsetu.in", 14, 16);

      // Document content
      pdf.setTextColor(17, 24, 39);
      pdf.setFontSize(18);
      pdf.setFont("helvetica", "bold");
      pdf.text(doc.name, 105, 32, { align: "center" });

      pdf.setFontSize(9);
      pdf.setFont("helvetica", "normal");
      pdf.setTextColor(107, 114, 128);
      pdf.text(`Generated on: ${formatDate(new Date())}  |  Document ID: DS${Date.now().toString().slice(-6)}`, 105, 39, { align: "center" });
      ...
      const allFields = doc.steps.flatMap((s) => s.fields);
      ...
      for (const step of doc.steps) {
        ...
        pdf.text(step.title.toUpperCase(), 18, y + 1);
        y += 10;
        ...
        for (const field of step.fields) {
          ...
          pdf.setFont("helvetica", "normal");
          pdf.setTextColor(17, 24, 39);
          const lines = pdf.splitTextToSize(val, 172);
          pdf.text(lines, 14, y);
          y += lines.length * 5 + 3;
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.setFont("helvetica", "normal");
        pdf.text(
          "Generated by DocSetu | docsetu.in | This document is based on expert-reviewed templates. Not legal advice.",
          105,
          291,
          { align: "center" }
        );
        pdf.text(`Page ${i} of ${pageCount}`, 196, 291, { align: "right" });
      }
